...
btnEnviar.onclick = function(){
    ...
    if(nome.valeu != "" && quantidade.valeu != "" && preco.valeu != "" && prateleira.value != "" && descricao.value != "" && categoria.value != ""){
        ...
        BD.push(produto);
        tabela.innerHTML += `<tr><td><input type="checkbox" id="${produto.id}" onchange="verificar(this.id)"></td><td>${produto.nome}</td><td>${produto.quantidade}</td><td>${preco.value}</td><td>${prateleira.value}</td><td>${descricao.value}</td><td>${categoria.value}</td></tr>`;
    } else{
        window.alert("Insira dados em todas as entradas necessarias!")
    }
...
btnExcluir.onclick = function(){
    for (let i = 0; i < BD.length; i++){
        ...
        if (elemento.checked){
            ...
            tabela.innerHTML = `<tr><td width="30px"></td><td>Nome</td><td>Quant.</td><td>Preço</td><td>Prateleira</td><td>Descricao</td><td>Categoria</td></tr>`;
            montarTabela();
        }
    }
}
...
function montarTabela(){
    for (let i = 0; i < BD.length; i++){
        tabela.innerHTML += `<tr><td width="30px"><input type="checkbox" id="${i}" onchange="verificar()"></td><td>${BD[i].nome}</td><td>${BD[i].quantidade}</td><td>${BD[i].preco}</td><td>${BD[i].prateleira}</td><td>${BD[i].descricao}</td><td>${BD[i].categoria}</td></tr>`;
    }
}
